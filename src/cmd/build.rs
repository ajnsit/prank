...
pub struct Build {
    ...
    #[arg(long, env = "PRANK_BUILD_HTML_OUTPUT")]
    pub html_output: Option<String>,

    /// Build in release mode
    #[arg(long, env = "PRANK_BUILD_RELEASE")]
    #[arg(default_missing_value="true", num_args=0..=1)]
    pub release: Option<bool>,

    /// Spago profile to use for building.
    #[arg(long, env = "PRANK_BUILD_SPAGO_PROFILE")]
    pub spago_profile: Option<String>,

    /// The output dir for all final assets
    #[arg(short, long, env = "PRANK_BUILD_DIST")]
    ...
    #[arg(long, env = "PRANK_BUILD_FROZEN")]
    #[arg(default_missing_value="true", num_args=0..=1)]
    pub frozen: Option<bool>,

    /// Require Spago.lock is up to date
    #[arg(long, env = "PRANK_BUILD_LOCKED")]
    #[arg(default_missing_value="true", num_args=0..=1)]
    pub locked: Option<bool>,

    /// The public URL from which assets are to be served
    #[arg(long, env = "PRANK_BUILD_PUBLIC_URL")]
    pub public_url: Option<BaseUrl>,

    /// Don't add a trailing slash to the public URL if it is missing
    #[arg(long, env = "PRANK_BUILD_PUBLIC_URL_NO_TRAILING_SLASH")]
    #[arg(default_missing_value="true", num_args=0..=1)]
    pub public_url_no_trailing_slash_fix: Option<bool>,

    /// Build without default features
    #[arg(long, env = "PRANK_BUILD_NO_DEFAULT_FEATURES")]
    #[arg(default_missing_value="true", num_args=0..=1)]
    pub no_default_features: Option<bool>,

    /// Build with all features
    #[arg(long, env = "PRANK_BUILD_ALL_FEATURES")]
    #[arg(default_missing_value="true", num_args=0..=1)]
    ...
    #[arg(short = 'M', long, env = "PRANK_BUILD_MINIFY")]
    #[arg(default_missing_value="true", num_args=0..=1)]
    pub minify: Option<bool>,

    /// Allows disabling sub-resource integrity (SRI)
    #[arg(long, env = "PRANK_BUILD_NO_SRI")]
    #[arg(default_missing_value="true", num_args=0..=1)]
    ...
    #[arg(long, env = "PRANK_BUILD_ALLOW_SELF_CLOSING_SCRIPT")]
    #[arg(default_missing_value="true", num_args=0..=1)]
...
impl Build {
    ...
    pub fn apply_to(self, mut config: Configuration) -> Result<Configuration> {
        ...
        config.build.spago_profile = spago_profile.or(config.build.spago_profile);
...
#[cfg(test)]
mod test {
    use crate::{Prank, PrankSubcommands};
    use clap::Parser;
    use rstest::rstest;

    #[rstest]
    #[case(&["prank", "build"], None)]
    #[case(&["prank", "build", "--no-default-features"], Some(true))]
    #[case(&["prank", "build", "--no-default-features", "true"], Some(true))]
    #[case(&["prank", "build", "--no-default-features", "false"], Some(false))]
    fn test_bool_no_arg(#[case] input: &[&str], #[case] expected: Option<bool>) {
        let cli = Prank::parse_from(input);
        let PrankSubcommands::Build(build) = cli.action else {
